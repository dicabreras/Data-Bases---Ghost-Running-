...
BEGIN
    ...
    SET @grant_sql = CONCAT('GRANT SELECT, INSERT, UPDATE, DELETE ON `', DATABASE(), '`.`Training` TO ', QUOTE(p_app_username), '@', QUOTE('%'));
    PREPARE stmt_grant FROM @grant_sql;
    EXECUTE stmt_grant;
    DEALLOCATE PREPARE stmt_grant;

    SET @grant_sql = CONCAT('GRANT SELECT, INSERT, UPDATE, DELETE ON `', DATABASE(), '`.`Publication` TO ', QUOTE(p_app_username), '@', QUOTE('%'));
    PREPARE stmt_grant2 FROM @grant_sql;
    EXECUTE stmt_grant2;
    DEALLOCATE PREPARE stmt_grant2;

    SET @grant_sql = CONCAT('GRANT SELECT, INSERT, UPDATE, DELETE ON `', DATABASE(), '`.`Comment` TO ', QUOTE(p_app_username), '@', QUOTE('%'));
    PREPARE stmt_grant3 FROM @grant_sql;
    EXECUTE stmt_grant3;
    DEALLOCATE PREPARE stmt_grant3;

    SET @grant_sql = CONCAT('GRANT SELECT, INSERT, UPDATE, DELETE ON `', DATABASE(), '`.`Route` TO ', QUOTE(p_app_username), '@', QUOTE('%'));
    PREPARE stmt_grant4 FROM @grant_sql;
    EXECUTE stmt_grant4;
    DEALLOCATE PREPARE stmt_grant4;

    SET @grant_sql = CONCAT('GRANT SELECT, INSERT, UPDATE, DELETE ON `', DATABASE(), '`.`Kilometer` TO ', QUOTE(p_app_username), '@', QUOTE('%'));
